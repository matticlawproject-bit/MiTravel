...
function decryptSecretFromStorage(payload) {
  ...
  try {
    ...
    if (!salt.length || !iv.length || !tag.length || !ciphertext.length) return '';
    const master = resolveMasterKeyMaterial();
    const key = crypto.scryptSync(master, salt, 32);
...
async function resolveStripeSecretKey() {
  const direct = String(process.env.STRIPE_SECRET_KEY || '').trim();
  if (direct) return direct;
  if (stripeSecretLoaded) return stripeSecretCache;
  stripeSecretLoaded = true;
  if (!dataStore || typeof dataStore.getSecret !== 'function') {
    stripeSecretCache = '';
    return stripeSecretCache;
  }
  const encrypted = await dataStore.getSecret(SECRET_NAMES.stripeSecretKey);
  stripeSecretCache = decryptSecretFromStorage(encrypted);
  return stripeSecretCache;
}
...
async function storeEncryptedSecret(secretName, clearValue) {
  ...
  const encrypted = encryptSecretForStorage(clearValue);
  if (!encrypted) {
    throw new Error('Secret value cannot be empty.');
  }
  await dataStore.setSecret(secretName, encrypted);
}
...
async function ensureDataStore() {
  if (dataStoreReadyPromise) {
    await dataStoreReadyPromise;
    return;
  }
  dataStore = createPgDataStore({
    defaults: DEFAULT_COLLECTIONS,
    legacyFiles: LEGACY_JSON_FILES
  });
  dataStoreReadyPromise = (async () => {
...
function readJson(collection) {
  ...
  return dataStore.read(collection);
}

function writeJson(collection, value) {
  if (!dataStore) {
    throw new Error('Data store is not initialized.');
  }
  dataStore.write(collection, value);
}
...
function buildAdminStats() {
  ...
  const totalSalesAmount = bookings.reduce((acc, booking) => acc + Number(booking.totalAmount || 0), 0);
  const totalAmountPaidToDuffel = bookings.reduce((acc, booking) => {
    const offerAmount = Number(booking?.flight?.offerTotalAmount);
    if (Number.isFinite(offerAmount) && offerAmount > 0) {
      return acc + offerAmount;
    }
    const baseAmount = Number(booking?.flight?.baseCashPrice);
  ...
  const stripe = bookings.reduce((acc, booking) => {
    const auth = booking?.stripeAuthorization && typeof booking.stripeAuthorization === 'object'
      ? booking.stripeAuthorization
      : null;
    const capture = booking?.stripeCapture && typeof booking.stripeCapture === 'object'
      ? booking.stripeCapture
      : null;
    ...
    if (authMode === 'live') {
      acc.authorizedLiveCount += 1;
    } else {
      acc.authorizedMockCount += 1;
    }

    const authAmount = Number(auth.amount ?? booking.totalAmount ?? 0);
    if (Number.isFinite(authAmount) && authAmount > 0) {
      acc.authorizedAmount += authAmount;
    }

    if (!capture) {
      if (auth.captureRequired) {
        acc.pendingCaptureCount += 1;
      }
      return acc;
    }

    const captureStatus = String(capture.status || '').trim().toLowerCase();
    const captured = Boolean(capture.captured) || captureStatus === 'succeeded' || captureStatus === 'processing';
    if (captured) {
      acc.capturedCount += 1;
      const capturedAmount = Number(capture.amount ?? authAmount);
      if (Number.isFinite(capturedAmount) && capturedAmount > 0) {
        acc.capturedAmount += capturedAmount;
      }
    } else if (capture.error) {
      acc.captureFailedCount += 1;
    } else if (auth.captureRequired) {
      acc.pendingCaptureCount += 1;
    }
  ...
  stripe.authorizedAmount = Number(stripe.authorizedAmount.toFixed(2));
  stripe.capturedAmount = Number(stripe.capturedAmount.toFixed(2));
  stripe.statusBreakdown = [
    { stage: 'Authorized', count: stripe.authorizedCount },
    { stage: 'Captured', count: stripe.capturedCount },
    { stage: 'Pending capture', count: stripe.pendingCaptureCount },
    { stage: 'Capture failed', count: stripe.captureFailedCount }
  ];
  const totalAmountCollected = Number(stripe.capturedAmount.toFixed(2));
  ...
  return {
    ...
    totalAmountPaidToDuffel: Number(totalAmountPaidToDuffel.toFixed(2)),
    totalFeeAmount: Number(totalFeeAmount.toFixed(2)),
    bookingAmount: Number(totalSalesAmount.toFixed(2)),
...
function mergeLoyaltyAccounts(primary, secondary) {
  const merged = [];
  const dedup = new Set();
  const all = [
  ...
  for (const account of all) {
    const airline = String(account?.airline_iata_code || '').trim().toUpperCase();
    const number = String(account?.account_number || '').trim();
    if (!airline || !number) continue;
    const fingerprint = `${airline}|${number}`;
    if (dedup.has(fingerprint)) continue;
    dedup.add(fingerprint);
    merged.push({
...
function sanitizeFlightResult(entry) {
  ...
  return {
    ...
    baseCashPrice: Number(entry.baseCashPrice || 0),
    offerTotalAmount: String(entry.offerTotalAmount || '').trim(),
    offerTotalCurrency: String(entry.offerTotalCurrency || entry.currency || '').trim(),
    currency: String(entry.currency || '').trim() || 'EUR',
    feeCabin: String(entry.feeCabin || '').trim(),
...
function extractPointsRequiredFromDuffelOffer(offer) {
  if (!offer || typeof offer !== 'object') return 0;

  const directCandidates = [
    offer.points_required,
    offer.miles_required,
    offer.loyalty_points_required,
    offer.total_points,
    offer.total_miles,
    offer?.payment_requirements?.points,
    offer?.payment_requirements?.miles
  ];
  ...
  // Fallback: inspect likely points/miles keys in the offer payload.
  const queue = [{ node: offer, depth: 0 }];
  while (queue.length) {
    const current = queue.shift();
    if (!current || current.depth > 4 || !current.node || typeof current.node !== 'object') continue;

    if (Array.isArray(current.node)) {
      for (const item of current.node) {
        if (item && typeof item === 'object') queue.push({ node: item, depth: current.depth + 1 });
      }
      continue;
    }

    for (const [rawKey, rawValue] of Object.entries(current.node)) {
      const key = String(rawKey || '').toLowerCase();
      const looksLikePoints =
        (key.includes('point') || key.includes('mile'))
...
function normalizeDuffelOffer(offer, fallback) {
  ...
  const totalAmountRaw = String(offer.total_amount || '').trim();
  ...
  return {
    ...
    pointsRequired: extractPointsRequiredFromDuffelOffer(offer),
    cashPrice: Number.isFinite(totalAmount) ? totalAmount : 0,
    offerTotalAmount: totalAmountRaw,
    offerTotalCurrency: offer.total_currency || 'USD',
...
function buildDuffelPaymentObject(amountValue, currency) {
  const amount = normalizeAmountString(amountValue);
  if (!amount) return null;
  const mode = String(process.env.DUFFEL_ORDER_PAYMENT_MODE || 'cash').trim().toLowerCase();
  if (mode === 'lodge_card' || mode === 'lodge-card') {
    const number = String(process.env.DUFFEL_LODGE_CARD_NUMBER || '4111111111111111').trim();
    const cvc = String(process.env.DUFFEL_LODGE_CARD_CVC || '123').trim();
    const month = Number(process.env.DUFFEL_LODGE_CARD_EXP_MONTH || 12);
    const year = Number(process.env.DUFFEL_LODGE_CARD_EXP_YEAR || 2030);
    const name = String(process.env.DUFFEL_LODGE_CARD_NAME || 'MiTravel Lodge Card').trim();
    return {
      type: 'card',
      currency,
      amount,
      card: {
        number,
        cvc,
        expiry_month: Number.isFinite(month) ? month : 12,
        expiry_year: Number.isFinite(year) ? year : 2030,
        name
      }
    };
  }

  return { type: 'balance', currency, amount };
}

async function createDuffelHoldOrderAndPay({
  offerId,
  passengerId,
  passenger
}) {
  const holdResponse = await fetch(`${DUFFEL_BASE_URL}/air/orders`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${DUFFEL_ACCESS_TOKEN}`,
      'Duffel-Version': DUFFEL_VERSION,
  ...
  if (!holdResponse.ok) {
    const detail = holdJson.errors?.[0]?.message || holdJson.error || 'Duffel hold order creation failed.';
    return { error: detail };
  }

  const orderId = String(holdJson?.data?.id || '').trim();
  const orderAmount = normalizeAmountString(holdJson?.data?.total_amount);
  const orderCurrency = String(holdJson?.data?.total_currency || '').trim();
  if (!orderId || !orderAmount || !orderCurrency) {
    return { error: 'Duffel hold order did not return payable total amount/currency.' };
  }

  const paymentObject = buildDuffelPaymentObject(orderAmount, orderCurrency);
  if (!paymentObject) {
    return { error: 'Invalid Duffel payment payload for hold order.' };
  }

  const payResponse = await fetch(`${DUFFEL_BASE_URL}/air/payments`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${DUFFEL_ACCESS_TOKEN}`,
      'Duffel-Version': DUFFEL_VERSION,
  ...
  if (!payResponse.ok) {
    const detail = payJson.errors?.[0]?.message || payJson.error || 'Duffel payment failed for hold order.';
    return { error: detail };
  }
  ...
  if (!verify.ok || !verifyJson?.data?.id) {
    const detail = verifyJson?.errors?.[0]?.message || verifyJson?.error || 'Duffel hold order verification failed.';
    return { error: detail };
  }
...
async function authorizePaymentWithStripe({ user, payment, flight }) {
  const amount = Number(flight.cashPrice || 0);
  if (!Number.isFinite(amount) || amount <= 0) {
    return { error: 'Invalid booking amount for Stripe authorization.' };
  }

  const currency = String(flight.currency || 'USD').trim().toLowerCase();
  ...
  if (!stripeSecret) {
    return {
      approved: true,
      mode: 'mock',
      authorizationId: `mock_auth_${crypto.randomBytes(6).toString('hex')}`,
      status: 'approved',
      amount,
  ...
  try {
    const route = `${String(flight.outboundFrom || flight.from || '').toUpperCase()}-${String(flight.outboundTo || flight.to || '').toUpperCase()}`;
    const airline = String(flight.outboundAirline || flight.airline || 'Unknown').trim();
    const returnAirline = String(flight.returnAirline || '').trim();
    const outboundFlightNo = String(flight.outboundFlightNumber || flight.flightNumber || '').trim();
    const returnFlightNo = String(flight.returnFlightNumber || '').trim();
    const cabin = String(flight.cabin || '').trim();
    ...
    form.set('metadata[offer_id]', String(flight.offerId || ''));
    form.set('metadata[profile_payment_id]', String(payment.id || ''));
    form.set('metadata[profile_payment_method]', String(payment.method || ''));
    form.set('metadata[route]', route);
    form.set('metadata[airline]', airline);
    form.set('metadata[return_airline]', returnAirline);
    form.set('metadata[outbound_flight_number]', outboundFlightNo);
    form.set('metadata[return_flight_number]', returnFlightNo);
    form.set('metadata[cabin]', cabin);
    form.set('metadata[travel_date]', travelDate);
    form.set('metadata[return_date]', returnDate);
    form.set('metadata[display_amount]', `${amount.toFixed(2)} ${currency.toUpperCase()}`);

    const response = await fetch(`${STRIPE_BASE_URL}/v1/payment_intents`, {
      method: 'POST',
      headers: {
    ...
    if (!response.ok) {
      const detail = json?.error?.message || json?.error || 'Stripe authorization failed.';
      return { error: detail };
    }

    const status = String(json.status || '').toLowerCase();
    if (!['requires_capture', 'succeeded'].includes(status)) {
      return { error: `Stripe authorization not approved. Current status: ${json.status || 'unknown'}.` };
    }

    return {
      approved: true,
      mode: 'live',
      authorizationId: String(json.id || ''),
      status: json.status || 'requires_capture',
      amount,
      currency: currency.toUpperCase(),
      paymentMethod: payment.method,
      captureRequired: true
    ...
    return { error: `Stripe authorization failed. ${error.message || 'Unknown error.'}` };
  }
}

async function captureStripeAuthorization({ stripeAuthorization, flight, duffelOrder }) {
  if (!stripeAuthorization || stripeAuthorization.error) {
    return { error: 'Missing Stripe authorization context for capture.' };
  }

  if (stripeAuthorization.mode !== 'live') {
    return {
      captured: true,
      mode: 'mock',
      captureId: `mock_capture_${crypto.randomBytes(6).toString('hex')}`,
      status: 'succeeded'
    };
  }
  ...
  const stripeSecret = await resolveStripeSecretKey();
  if (!stripeSecret) {
    return { error: 'Stripe secret key is missing for capture.' };
  }

  const paymentIntentId = String(stripeAuthorization.authorizationId || '').trim();
  if (!paymentIntentId) {
    return { error: 'Missing Stripe PaymentIntent id for capture.' };
  }

  const amountMinor = Math.round(Number(flight.cashPrice || 0) * 100);
  if (!Number.isFinite(amountMinor) || amountMinor <= 0) {
    return { error: 'Invalid capture amount.' };
  }

  try {
    const form = new URLSearchParams();
    form.set('amount_to_capture', String(amountMinor));
    form.set('metadata[offer_id]', String(flight.offerId || ''));
    form.set('metadata[duffel_order_id]', String(duffelOrder?.orderId || ''));
    form.set('metadata[duffel_booking_reference]', String(duffelOrder?.bookingReference || ''));

    const response = await fetch(`${STRIPE_BASE_URL}/v1/payment_intents/${encodeURIComponent(paymentIntentId)}/capture`, {
      method: 'POST',
      headers: {
        Authorization: `Bearer ${stripeSecret}`,
        'Content-Type': 'application/x-www-form-urlencoded'
    ...
    const json = await response.json();
    if (!response.ok) {
      const detail = json?.error?.message || json?.error || 'Stripe capture failed.';
      return { error: detail };
    }

    const status = String(json.status || '').toLowerCase();
    if (!['succeeded', 'processing'].includes(status)) {
      return { error: `Stripe capture not completed. Status: ${json.status || 'unknown'}.` };
    }

    return {
      captured: true,
      mode: 'live',
      captureId: String(json.id || paymentIntentId),
      status: json.status || 'succeeded',
      amountCaptured: Number(json.amount_received || amountMinor) / 100,
      currency: String(json.currency || '').toUpperCase()
    };
  } catch (error) {
    return { error: `Stripe capture failed. ${error.message || 'Unknown error.'}` };
  }
}

async function fetchDuffelOfferPricing(offerId) {
  const id = String(offerId || '').trim();
  if (!id || !DUFFEL_ACCESS_TOKEN) return null;

  try {
    const response = await fetch(`${DUFFEL_BASE_URL}/air/offers/${id}`, {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${DUFFEL_ACCESS_TOKEN}`,
        'Duffel-Version': DUFFEL_VERSION,
    ...
    const amount = normalizeAmountString(json?.data?.total_amount);
    const currency = String(json?.data?.total_currency || '').trim();
...
async function createDuffelOrderForOffer(user, flight) {
  ...
  const livePricing = await fetchDuffelOfferPricing(offerId);
  const currency = String(livePricing?.currency || flight.offerTotalCurrency || flight.currency || 'GBP');
  // Duffel must receive the original offer amount exactly as returned by offer search.
  const offerTotalAmount = normalizeAmountString(livePricing?.amount || flight.offerTotalAmount);
  const feeAmount = Number(flight.feeAmount || 0);
  const derivedBase = Number(flight.cashPrice || 0) - (Number.isFinite(feeAmount) ? feeAmount : 0);
  const baseAmount = Number(flight.baseCashPrice || (Number.isFinite(derivedBase) ? derivedBase : 0) || flight.cashPrice || 0);
  const duffelAmount = offerTotalAmount || (Number.isFinite(baseAmount) && baseAmount > 0 ? baseAmount.toFixed(2) : '');
  if (!duffelAmount) {
    return { error: 'Invalid offer amount for Duffel order.' };
  }
  const duffelPayment = buildDuffelPaymentObject(duffelAmount, currency);
  if (!duffelPayment) {
    return { error: 'Invalid Duffel payment payload amount.' };
  }
  ...
  if (!response.ok) {
    ...
    return { error: `${detail} (sent ${duffelAmount} ${currency})` };
  }
...
async function handleApi(req, res, pathname) {
  ...
  if (req.method === 'POST' && pathname === '/api/bookings') {
    ...
    const stripeAuthorization = await authorizePaymentWithStripe({ user, payment, flight });
    if (stripeAuthorization.error) {
      sendJson(res, 400, { error: stripeAuthorization.error });
      return;
    }
    ...
    const stripeCapture = await captureStripeAuthorization({
      stripeAuthorization,
      flight,
      duffelOrder
    });
    if (stripeCapture.error) {
      sendJson(res, 400, { error: `Duffel order created but Stripe capture failed: ${stripeCapture.error}` });
      return;
    }
  ...
  if (req.method === 'POST' && pathname === '/api/flights/search') {
    ...
    const finalPayload = {
      ...
      loyaltyAccounts: mergeLoyaltyAccounts(body.loyaltyAccounts, profileLoyaltyAccounts)
    };
  ...
  if (req.method === 'POST' && pathname === '/api/flights/ai-search') {
    ...
    const profileLoyaltyAccounts = buildDuffelLoyaltyAccountsForUser(user.id);
    const requestLoyaltyAccounts = Array.isArray(body.loyaltyAccounts) ? body.loyaltyAccounts : [];
    ...
    const finalPayload = {
      ...
      preferences: Array.isArray(body.preferences) ? body.preferences : (Array.isArray(user.preferences) ? user.preferences : []),
      loyaltyAccounts: mergeLoyaltyAccounts(requestLoyaltyAccounts, profileLoyaltyAccounts)
    };
